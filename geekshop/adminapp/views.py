...
from authapp.forms import UserShopRegisterForm
from adminapp.forms import UserShopAdminEditForm, ProductCategoryEditForm


@user_passes_test(lambda u: u.is_superuser)
def users(request):
    title = 'админка/пользователи'
    users_list = UserShop.objects.all().order_by('-is_active', '-is_superuser', '-is_staff', 'username')
    context = {
        'title': title,
        'objects': users_list
    }
...
def user_create(request):
    ...
    if request.method == 'POST':
        user_form = UserShopRegisterForm(request.POST, request.FILES)
        if user_form.is_valid():
            user_form.save()
            return HttpResponseRedirect(reverse('admin_stuff:users'))
    else:
        user_form = UserShopRegisterForm()
...
def user_update(request, pk):
    ...
    edit_user = get_object_or_404(UserShop, pk=pk)
    if request.method == 'POST':
        edit_form = UserShopAdminEditForm(request.POST, request.FILES, instance=edit_user)
        if edit_form.is_valid():
            edit_form.save()
            return HttpResponseRedirect(reverse('admin_stuff:user_update', args=[edit_user.pk]))
    else:
        edit_form = UserShopAdminEditForm(instance=edit_user)
...
def user_delete(request, pk):
    ...
    user_to_delete = get_object_or_404(UserShop, pk=pk)
    if request.method == 'POST':
